        ...
        this.discount_selected.categories.forEach(categorie_selected => {
          this.categories.forEach(categorie => {
            if(categorie._id == categorie_selected._id){
              this.categories_selected.push(this.categorie);
            }
          });
        });
      }
  ...
  save(){
    ...
    if(this.type_segment == 1){
      if(this.products_selected.length == 0){
        this.toaster.open(NoticyAlertComponent, {text: `danger-'Upps! TIENE QUE SELECCIONAR UN PRODUCTO AL MENOS'`});
        return;
      }
    }
    if(this.type_segment == 2){
      if(this.categories_selected.length = 0){
        this.toaster.open(NoticyAlertComponent, {text: `danger-'Upps! TIENE QUE SELECCIONAR UN CATEGORIA AL MENOS'`});
        return;
      }
    }
